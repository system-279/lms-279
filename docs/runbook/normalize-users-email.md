...
- PR #277 / #280 / #284 マージ済み（allowed_emails 側の正規化・棚卸し完了）
...
2. 進捗データ（`user_progress`, `course_progress`, `quiz_attempts`, `video_events`, `lesson_sessions`）の
   userId を残す側に手動マージ（必要に応じて個別スクリプトを用意）
...
本スクリプトは「正規化前後で衝突する」ケースのみ検出する。
「既に両方正規化済みだが同一 email」という重複は検出しない。この場合は
`scripts/audit-users-vs-allowed-emails.ts`（Issue #279）や別途の監査スクリプトで検出し、
同様の手動マージ手順で解消する。
