...
完了通知 cron `dxcollege-completion-notifications` が毎時 00 分起動なので、進捗レーンは **毎時 30 分起動** にして同時起動時の Gmail 429 競合を回避する (impl-plan §4 / リスク表 §5)。
...
- `PERMISSION_DENIED`: 作成者 (cli 実行者) に `roles/cloudscheduler.admin` がない、または SA `dispatch-scheduler@...` に `roles/run.invoker` がない → IAM 確認
...
- `scaleTriggerExceeded`: 全テナント合計 300 名超で `true` (Cloud Tasks 移行検討、impl-plan §4.3)
...
- 完了通知レーンと進捗レーンの合算で Workspace `system@279279.net` の Gmail 1800 通/日 (rolling 24h、impl-plan §4.3) を超過すると **両レーン同時停止** に直結する
- 300 名超は impl-plan §4.3 で Cloud Tasks 移行検討の閾値として明示済
