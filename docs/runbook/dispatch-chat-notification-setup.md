...
> **2026-09-18 実施済み**: 本手順書の §1〜§4 を全て実施・実機確認済み。Secret 2本の作成
> （開発者が対話的に `gcloud secrets versions add` を実行、投入時にアカウントが
> `hy.unimail.11@gmail.com` になっており `PERMISSION_DENIED` となる事故が発生 → `gcloud config
> set account system@279279.net` で切替後に成功。複数プロジェクトを横断運用する環境での
> アクティブアカウント不一致は既知のリスクパターンであり、事前に `gcloud auth list` で
> `*` の付いたアクティブアカウントを確認してから実行するのが安全）、IAM 権限付与、
> Cloud Run env var の反映確認（`main` push で自動デプロイ済み）、実際の webhook への
> テスト投稿（完了通知/進捗レポート両レーン、HTTP 200・Chat スペースへの到達を
> スクリーンショットで確認）まで完了している。
