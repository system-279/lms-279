...
| 6 | 対象一覧レビュー | ❌ | ✅ |
| 7 | 本番有効化の技術ゲート認可 | ❌ | ✅ 開発者 |
| 8 | enabled=true 切替 (Web UI) | ⚠️ 状態確認支援のみ | ✅ **業務スーパー管理者本人** が UI 操作 |
| 9 | 次の cron で初回送信 | - 自動 - | - |
| 10 | audit_logs / run_history 確認 | ✅ | - |
| 11 | 受信者問い合わせ受付 | ❌ | ✅ 開発者 |
| 12 | 問題発生時 kill switch (Web UI) | ⚠️ 発動判断の助言のみ | ✅ **業務スーパー管理者本人** が UI 操作 |

> **運用方針 (2026-05-24 確定)**: 本番投入の最終操作 (Step 8 マスタートグル ON / Step 12 kill switch) は **業務スーパー管理者 (開発者でないスーパー管理者) 本人の手** でのみ実行する。AI も開発者も代行しない。これは AI 駆動開発 4 原則 §1 (AI は executor、人間は decision-maker) の本プロジェクトでの具体展開。Step 7 (技術ゲート認可) は引き続き開発者領分。
...
## Step 8: enabled=true 切替 (業務スーパー管理者 UI 操作、Step 7 認可後)

### 担当: 業務スーパー管理者 (Web UI 操作) + AI (前後の状態確認支援)

**運用方針 (2026-05-24 確定)**: 本番投入の最終操作は業務スーパー管理者本人の手でのみ実行する。AI も開発者も代行しない。Step 7 (技術ゲート認可) を開発者から受領した後、業務スーパー管理者へヘルプ URL (`/help/super#super-dispatch-settings`) を共有 → 業務スーパー管理者が UI を理解し「自分で操作できる」とご判断頂けた段階で、ご本人が実 write を行う。

### 手順 (業務スーパー管理者向け)

1. ブラウザで `/super/dispatch-settings` を開く
2. 画面最上部の「配信を有効化」スイッチを **OFF → ON** に切替
3. 「保存」ボタン押下
4. version が +1 されていること、「保存しました」が表示されることを確認

### AI の支援範囲

- 切替前: 現状 enabled / dry-run 対象一覧 (`gh workflow run dispatch-dry-run.yml`) の取得
- 切替後: enabled=true / version +1 / 監査ログに `settings_updated` イベント記録の確認
- 業務スーパー管理者からの操作・文言に関する質問への回答

### AI が代行しない操作 (executor 越権防止)

- 業務スーパー管理者の代わりに enabled=true を切替・保存すること
- 「業務スーパー管理者の判断を待たずに進める」意思決定

> 将来 AI 経由 (admin SDK workflow 例: `dispatch-settings-toggle.yml`) で切替可能にする選択肢もあるが、本運用方針は **「本番投入の最終操作は業務スーパー管理者本人」** に限定する設計のため、AI からの切替経路は **意図的に整備しない**。
...
## Step 12: 問題発生時の kill switch (業務スーパー管理者 UI 操作)

### 担当: 業務スーパー管理者 (Web UI 操作) + AI (発動判断の助言)

Step 8 と同様、本番投入の最終操作 (停止操作も含む) は業務スーパー管理者本人の領分。AI は audit_logs / run_history を確認して影響範囲を把握し、発動判断の助言を行う。
...
業務スーパー管理者がブラウザで `/super/dispatch-settings` の `enabled` を **true → false** に切替して保存。
