...
| 6 | 対象一覧レビュー | ❌ | ✅ |
| 7 | 本番有効化の明示認可 | ❌ | ✅ |
| 8 | enabled=true 切替 | ⚠️ Step 7 認可後 AI | - |
| 9 | 次の cron で初回送信 | - 自動 - | - |
| 10 | audit_logs / run_history 確認 | ✅ | - |
| 11 | 受信者問い合わせ受付 | ❌ | ✅ |
| 12 | 問題発生時 kill switch | ✅ | - |
...
## Step 8: enabled=true 切替 (AI 主導、Step 7 認可後)

### 手順

開発者がブラウザで `/super/dispatch-settings` を開き、`enabled` を **true** に切替して保存。

(代替: AI が super-admin API 経由で実行する場合は新規 admin SDK workflow が必要。現状未整備のため Web UI 経由が現実的)
...
## Step 12: 問題発生時の kill switch (AI 主導)
...
(または AI が super-admin API 経由で実行、要 Web UI 経由 or 新規 admin SDK workflow)
