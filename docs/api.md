...
### 受講者進捗 PDF 出力（Super Admin）

ベースURL: `/api/v2/super/`

スーパー管理者がテナント内の特定受講者の進捗をまとめた PDF をダウンロードする（ADR-032 Phase 1）。

| メソッド | パス | 説明 |
|---------|------|------|
| POST | `/tenants/:tenantId/users/:userId/progress-pdf` | 進捗 PDF 生成・ダウンロード |

#### リクエスト
```json
{
  "requestId": "uuid-v4",       // Phase 1 ではログ識別子、Phase 2 で idempotency key
  "sections": {                  // 7 フラグすべて必須 (boolean)
    "profile": true,
    "deadline": true,
    "summary": true,
    "lessons": true,
...
#### レスポンス
- 200: `application/pdf` (Buffer)、`Content-Disposition: attachment; filename="progress-<name>-<yyyy-mm-dd>.pdf"`
- 400: `invalid_tenant_id` / `demo_tenant_not_supported` / `invalid_user_id` / `bad_request` / `invalid_request_id` / `invalid_sections`
- 404: `tenant_not_found` / `user_not_in_tenant`
- 413: `pdf_too_large`（5MB 超）
- 500 / 503: `pdf_generation_failed`

#### 認可
- `superAdminAuthMiddleware` 配下
- `tenantId` は英数字+ハイフン+アンダースコアのみ許可（パストラバーサル防止）
- `tenantId === "demo"` は 400 で拒否（共有 InMemoryDataSource では越境チェックがすり抜けるため）
- `userId` は対象 tenant に所属することを Firestore tenant-scope の `getUserById` 経由で検証、null なら 404

関連 ADR: ADR-032（採用判断・推奨ペース計算境界仕様）
