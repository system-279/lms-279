...
Issue #272 Phase 1.8 セキュリティ先行実装 PR #277 を作成・レビュー対応・CI green 済。マージ待ち。並行して H-B (設計判断) を Issue #278 に分離。Phase 1.1–1.3 の GCP Console 操作は引き続きユーザー側作業待ち。
...
| # | Title | 意味 |
|---|-------|------|
| **#278** (新規, P0) | [ADR-031] allowed_emails の認可境界責務定義と既存 users 経路の再チェック | Codex 指摘 H-B の分離。ADR-031 改訂 + tenant-auth.ts 改修を伴うため別PR |
...
| Issue #278 の責務選択 (案A / 案B) | allowed_emails の責務を招待リスト vs 継続的認可境界のどちらにするか | tenant-auth.ts 改修の方針決定 |
