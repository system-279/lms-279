...
- [ ] Stage 6: ADR-040新規+ADR-019/027/036/020改訂+ドキュメント更新（PR #608 main merge済み）+ 重複行整理Phase A（読み取り専用監査スクリプト、PR #609 main merge済み。plan mode+codex plan reviewでセカンドオピニオン取得、実装後codex review 2件反映）は完了。次アクション: `gh workflow run audit-duplicate-synthetic-sessions.yml -f tenant_id=<tenant>` で本番監査を実行し実測データを得る（本セッションはローカルADC失効のため未実施）。Phase B（実際の統合/削除、実測結果を見てから設計）は未着手
