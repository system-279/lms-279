...
承認済み・実装完了（Stage 1〜6 すべて完了。Stage 5 は PR #619(2026-08-20)で本番 flag を `QUIZ_REQUIRE_ACTIVE_SESSION=true` へ切替、Cloud Run 実機で反映確認済み。ロールバック用 PR #620 を待機状態(未 merge)で保持。Stage 6 Phase B の safe グループ2件は開発者(super-admin)が2026-08-20に個別調査し「現状維持」と判断、対応完了）
...
1. **safe な2グループ（atali82iテナント）は super-admin が手動調査で対応**。監査スクリプトは PII 制限（userId/doc id 非出力）により対象ドキュメントを特定できないため、`docs/runbook/stage6-mixed-session-duplicate-cleanup.md` の手順に沿って開発者(super-admin)が個別に特定・判断した（2026-08-20実施。事前準備としてFirestore PITRを有効化しバックアップ経路を確保）。**両グループとも同一パターン**（real の未完了セッション複数件〔いずれも`force_exited`、`quizAttemptId=null`〕+ ケースD後方互換経由の `synthetic_pass` 完了記録1件）であり、real 側は実際に発生した入退室イベント、synthetic_pass 側は正当な合格記録と判断できたため、**削除・補正いずれも行わず現状維持と決定**（doc数の変更なし）。Stage 5 のケースD厳格化により、この種の real+synthetic 混在パターンは今後新規発生しない。
