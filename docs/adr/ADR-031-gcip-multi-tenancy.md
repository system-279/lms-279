...
| UID 紐付けの原子性 | ✅ **Issue #313 / PR #315（既存 user CAS）+ Issue #316（初回 create race 解消）で完全対応済み**。`setUserFirebaseUidIfUnset` CAS と `findOrCreateUserByEmailAndUid` atomic を DataSource I/F に追加。Firestore は `runTransaction` + sentinel doc (`tenants/{tid}/user_email_locks/{sha256(email)}`) で同一 email の create を直列化、InMemory は単一スレッド同期実行で同等セマンティクス。email fallback / 初回 create いずれも、既存 UID と異なる UID は 403 `uid_reassignment_blocked` で拒否、`platform_auth_error_logs` に tenant 横断で記録 | 維持。Sub-Issue H Staging 検証で以下を **明示判断**する: (a) `ABORTED` (transaction retry 上限超過時) を現状 401 で許容するか、503 + Retry-After に変更するか — 現状は `tenantAwareAuthMiddleware` の firebase catch 節で構造化ログ出力 + `req.user` 未設定 → 後段 `requireUser` で 401。同一 email への 5 並行初回ログインという稀ケース。クライアントが 401 でログアウト誘導する設計の場合は誤分類になるため、UX 観点で 503 化を要再検討 |
