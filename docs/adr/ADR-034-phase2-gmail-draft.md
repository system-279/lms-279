...
- Status: **Proposed** (impl-plan 承認済、実装着手中)
- Date: 2026-05-14
- Deciders: system-279, sanwaminamihonda@gmail.com
- 関連: ADR-032 (Phase 1 採択), ADR-033 (Rejected — SMTP relay 案), Issue #346 (Phase 2 起票)
...
[FE]                                         [BE]                            [Google]
 ...
 │                                            ├─ 越境チェック (Phase 1 と同じ)
 │                                            ├─ tenant.ownerEmail 取得
 │                                            ├─ PDF 生成 (Phase 1 ロジック再利用)
 │                                            ├─ MIME multipart 組み立て
 │                                            │   (Subject / From: 'me' / To: ownerEmail / Body / PDF 添付)
...
- **To**: `tenants/{tenantId}.ownerEmail` を自動入力 (未設定なら FE で「下書き作成」ボタンを disabled + ツールチップ表示)
- **Cc / Bcc**: Phase 2 では未対応 (スーパー管理者が Gmail UI で手動追加)
...
// tenants/{tenantId}/pdf_draft_logs/{requestId}
{
  ...
  ownerEmailHash: string;       // sha256(ownerEmail)
  draftId: string | null;       // Gmail draft ID (成功時)
  status: "success" | "failed";
  errorCode: string | null;     // gmail_scope_required / gmail_quota_exceeded / etc.
...
- PII 最小化: 受信者メールはハッシュのみ、PDF 内容は保存しない
...
| 400 | `owner_email_not_set` | ownerEmail 未設定 | ボタン disabled (本来到達しない、二重防御) |
