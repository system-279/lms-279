# ADR-026: Google Workspace連携（Drive動画インポート + Docsテスト生成）

## ステータス
承認済み

## コンテキスト
管理者がレッスンのコンテンツを登録する際、現在はGCSパスの直接指定またはファイルアップロードが必要で、テストは手動作成のみ。279279.netドメインのGoogle Workspaceにある動画やドキュメントを直接活用したいという要件がある。
...
2. **Google Docsテスト自動生成**: Docsの内容をVertex AI（Gemini）で分析し、テスト問題を自動生成
...
5. 管理者がプレビュー・編集後、既存のテスト保存APIで確定
