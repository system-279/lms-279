...
- 設計仕様書 / 実装計画 / ADR / handoff / コードコメント / PR 本文 / Issue / 監査ログ等すべてのドキュメントとコード内テキストで、本プロジェクトのプロダクトオーナー / 意思決定者 / プロジェクト発注者を指す場合は **「開発者」** と記載する（個人名・敬称・組織名を書かない）
- 既存ドキュメントの「本田様」表記は、当該ファイルを編集する機会に併せて「開発者」へ書き換える（無理に一括置換はしない、混在期間を許容）
- 役割名で書く必要がある場合は「開発者」「スーパー管理者」「テナント管理者」「受講者」を使い分ける
- グローバル memory への新規 feedback を書く際は、CLAUDE.md `feedback_project_runtime_paths.md` 既定どおり、本プロジェクト固有の役割名・固有運用情報を書かない
