...
describe("PATCH /api/v2/super/tenants/:id — GCIP フィールドバリデーション (Issue #312)", () => {
  ...
  it("既存 gcipTenantId が null のテナントで useGcip: true のみ指定すると 400 gcip_tenant_id_required (Case a)", async () => {
    const app = await buildApp();
    const res = await supertest(app)
      .patch("/api/v2/super/tenants/tenant-a")
      .set("X-User-Email", "super@example.com")
  ...
  it("既存 useGcip=true テナントで gcipTenantId: null のみ PATCH すると 400 gcip_tenant_id_required (Case b)", async () => {
    // 既存 GCIP 有効化済み（useGcip=true, gcipTenantId="gcip-xyz"）
    mockTenantDocGet.mockResolvedValue({
      exists: true,
      data: () => ({
        id: "tenant-a",
    ...
    const res = await supertest(app)
      .patch("/api/v2/super/tenants/tenant-a")
      .set("X-User-Email", "super@example.com")
      .send({ gcipTenantId: null });

    expect(res.status).toBe(400);
    expect(res.body.error).toBe("gcip_tenant_id_required");
    // Firestore 書き込みは発生しないこと（GCIP 認証経路が破壊されない）
