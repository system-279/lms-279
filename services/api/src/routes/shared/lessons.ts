...
router.get("/lessons/:lessonId", requireUser, async (req: Request, res: Response) => {
  ...
  // hasQuiz も必須（2回目の codex review 指摘反映: hasVideo=true/hasQuiz=false のレッスンで
  // 「テストの受験にはセッションが必要」と表示すると、そもそも受けるテストが存在しないのに
  // 誤案内になる）。
  const sessionRequired = isQuizActiveSessionRequired() && lesson.hasVideo && lesson.hasQuiz;
