...
function buildMockDb(seed?: Record<string, unknown>) {
  ...
  if (seed) {
    docState.set("quiz_policy", { exists: true, data: () => seed });
  }

  const docRef = {
    get: vi.fn(async () => docState.get("quiz_policy") ?? { exists: false, data: () => ({}) }),
    set: vi.fn(async (data: Record<string, unknown>) => {
      docState.set("quiz_policy", { exists: true, data: () => data });
    }),
  };
