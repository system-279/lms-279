 ...
 *
 * 最もリスクの高い3パスを検証:
 * 1. セッション期限切れ → クイズ提出拒否（403）
 * 2. セッションなし → クイズ提出は通常通り成功（後方互換）
 * 3. セッションあり+有効期限内 → クイズ提出成功+セッション完了（退室打刻）
 */
...
describe("Quiz submission × Session integration", () => {
  ...
  // =========================================================
  // 1. 後方互換: セッションなしでもクイズ提出は成功する
  // =========================================================
  it("セッションなしでもクイズ提出が成功する（後方互換）", async () => {
    // attempt作成
  ...
  // =========================================================
  // 2. 有効セッション内でクイズ提出 → 成功+セッション完了
  // =========================================================
  it("有効セッション内でクイズ提出するとセッションが完了する", async () => {
    // セッション作成
  ...
  // =========================================================
  // 3. 期限切れセッションでクイズ提出 → 403拒否
  // =========================================================
  it("期限切れセッションではクイズ提出が403で拒否される", async () => {
    // セッション作成
    const sessionRes = await studentRequest
      .post("/lesson-sessions")
      .send({
