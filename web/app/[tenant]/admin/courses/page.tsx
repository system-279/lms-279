...
export default function CoursesPage() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      const data = await authFetch<{ courses: Course[] }>(
        `/api/v1/admin/courses${query}`
      );
  ...
  const handleEdit = async () => {
    ...
    setEditError(null);
    try {
      await authFetch(`/api/v1/admin/courses/${editingCourse.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    ...
    setDeleteError(null);
    try {
      await authFetch(`/api/v1/admin/courses/${deletingCourse.id}`, {
        method: "DELETE",
      });
      setDeleteOpen(false);
  ...
  const handlePublish = async (course: Course) => {
    try {
      await authFetch(`/api/v1/admin/courses/${course.id}/publish`, {
        method: "PATCH",
      });
      fetchCourses();
    } catch (e) {
  ...
  const handleArchive = async (course: Course) => {
    try {
      await authFetch(`/api/v1/admin/courses/${course.id}/archive`, {
        method: "PATCH",
      });
      fetchCourses();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setCreateName(""); setCreateDescription(""); setCreateError(null); setCreateOpen(true); }}>
          新規作成
        </Button>
      </div>
              ...
              <TableRow key={course.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2 flex-wrap">
                    <Link href={`/${tenantId}/admin/courses/${course.id}/lessons`}>
                    ...
                    {course.status !== "published" && (
                      <Button variant="outline" size="sm" onClick={() => handlePublish(course)}>
                        公開
                      </Button>
                    )}
                    {course.status !== "archived" && (
                      <Button variant="outline" size="sm" onClick={() => handleArchive(course)}>
                        アーカイブ
                      </Button>
                    )}
                    <Button
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            「{deletingCourse?.name}」を削除しますか？この操作は取り消せません。
          </p>
          {deleteError && (
